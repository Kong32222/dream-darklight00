...
void CDRMReceiver::SetInputDevice(string s)
{
    //    std::cout<<"InputDevice dev = "<< s <<std::endl;
    ReceiveData.Stop();
...
void CDRMReceiver::SetOutputDevice(string device)
{
...
void CDRMReceiver::EnumerateInputs(vector<string>& names,
                                   vector<string>& descriptions,
                                   string& defaultInput)
{
...
void CDRMReceiver::DemodulateDRM(bool& bEnoughData)
{
    if(1)
    {
        //查看 鲁棒模式 频谱占用
        CParameter* ptemp = this->GetParameters();
        if(ptemp == nullptr)
        {
            cout<<" nullptr!"<<endl;
        ...
        {
            //            int mode = ptemp->GetWaveMode();
            //            int interx = ptemp->GetSpectrumOccup();
            // mode  = 1 (B); interx  = 3
            // cout<<"mode  = "<<mode << ";interx  = "<< interx<<endl;
        }
    }
    ...
    int mysize = 0 ;
    int inputsize = 0;
    //现象是：640 1280  (640是 源码在初始化的时候 设置成了一半 防止溢出)

    //demodul input InputBlockSize = 1280, 已经有的数据 GetFillLevel: 1280
    //demodul input InputBlockSize = 1280, 已经有的数据 GetFillLevel: 640
    //    cout<<"demodul重采样 InputBlockSize= "<< InputResample.GetInputBlockSize()
    //      << ", 已经有的数据 GetFillLevel: "<<DemodDataBuf.GetFillLevel()  << endl;
    //(输入重采样)  InputBuffer.GetFillLevel() >= this->iInputBlockSize
    if (InputResample.ProcessData(Parameters, DemodDataBuf, InpResBuf))
    {
        bEnoughData = true;
    }

    // cout<<"InpResBuf outputSize:"<<InpResBuf.GetFillLevel()<<endl;
    //size 和上面的一致
    /* (频率同步获取) Frequency synchronization acquisition -------------------- */
    /*
        FreqSyncAcq  GetInputBlockSize  = 1280;
        FreqSyncAcq  GetOutputBlockSize =  0;   // 异常  因为:(provessData internal :FreqSyncAcq output size = 0)
        输出的数据 GetFillLevel: 0                // 异常
    */
    if (FreqSyncAcq.ProcessData(Parameters, InpResBuf, FreqSyncAcqBuf))
    {
        bEnoughData = true;
    }
    //    cout<< "频率同步Freq GetInputBlockSize: "<< FreqSyncAcq.GetInputBlockSize()
    //        << "; Freq GetOutputBlockSize: " << FreqSyncAcq.GetOutputBlockSize()
    //        << ", 输出的数据GetFillLevel: "<<FreqSyncAcqBuf.GetFillLevel() <<endl;


    /*cout<<"--------------------------------------------------------------------------"<< endl;
     * timesync   1280
        TimeSync  inputsize  = 1280 or 1279;(需求)
        TimeSync  outputsize = 1024;
        TimeSyncBuf FillLevel: 0    // 1024  0 ,1024  0 ,1024  0
        TimeSync InputBlockSize: 1280; Freq OutputBlockSize: 1024 TimeSyncBuf FillLevel: 0
        (过 一段时间)
        TimeSync InputBlockSize: 1280; Freq OutputBlockSize: 1024;TimeSyncBuf FillLevel: 1024
    */
    ...
    //    cout<< "时间同步 TimeSync InputBlockSize: " << TimeSync.GetInputBlockSize()
    //        << "; Freq OutputBlockSize: " << TimeSync.GetOutputBlockSize()
    //        <<";TimeSyncBuf FillLevel: "<<TimeSyncBuf.GetFillLevel() <<endl;

    /*
     * OFDMDemodulation size: 1024; OFDMDemodulation output size: 207
     *   表48：不同鲁棒模式下的载波数  鲁棒模式:B, 频 谱 占 用: 3
        OFDMDemodulation   Inputsize: 1024;
        OFDMDemodulation   Outputsize: 207;   ( 207 -1 )/2 = 103 ,-103
    */
    //    cout<< "OFDMDemodulation size: "<< OFDMDemodulation.GetInputBlockSize()
    //        << "; OFDMDemodulation output size: "<< OFDMDemodulation.GetOutputBlockSize()<<endl;
    /* (OFDM 解调) OFDM-demodulation ---------------------------------------- */
    if (OFDMDemodulation.ProcessData(Parameters, TimeSyncBuf, OFDMDemodBuf))
    {
        //cout<<"第4步:OFDM 解调 成功"<<endl;
        bEnoughData = true;
    }

    // int inputsize5 = OFDMDemodBuf.GetFillLevel();
    mysize = SyncUsingPil.GetInputBlockSize();

    /* (频域同步) Synchronization in the frequency domain (using pilots) --- */
    if (SyncUsingPil.ProcessData(Parameters, OFDMDemodBuf, SyncUsingPilBuf))
    {
        // cout<<"第5步:频域同步 成功"<<endl;
        bEnoughData = true;
    }

    inputsize = SyncUsingPilBuf.GetFillLevel();
    mysize = ChannelEstimation.GetInputBlockSize();

    /* (信道估计与均衡) Channel estimation and equalisation 信道估计和均衡 ---------------------- */
    if (ChannelEstimation.
            ProcessData(Parameters, SyncUsingPilBuf, ChanEstBuf))
    {
        bEnoughData = true;
        // cout<<"第6步:信道估计与均衡 成功"<<endl;
    ...
    inputsize = ChanEstBuf.GetFillLevel();
    mysize = OFDMCellDemapping.GetInputBlockSize();

    /* (MSC、FAC、SDC 与导频解映射)  Demapping of the MSC, FAC, SDC and pilots off the carriers */
    if (OFDMCellDemapping.ProcessData(Parameters, ChanEstBuf,
                                      MSCCarDemapBuf,
                                      FACCarDemapBuf, SDCCarDemapBuf))
    {
        // cout<<"第7步: MSC、FAC、SDC 与导频解映射 成功"<<endl;
        bEnoughData = true;
    }
...
CDRMReceiver::DecodeDRM(bool& bEnoughData, bool& bFrameToSend)
{
    ...
    //FACMLC InputBlockSize: 65, OutputBlockSize: 72; FACCarDemapBuf FillLevel: 0
    //FACCarDemapBuf FillLevel:  目前大小一直为 0

    //  FACMLCDecoder InputBlockSize: 65;FACMLCDecoder OutputBlockSize: 72;
    // FACCarDemapBuf FillLevel:  32; FACDecBuf
    // FACCarDemapBuf FillLevel:  28; FACDecBuf
    // FACCarDemapBuf FillLevel:  33
    // 结论是  FACCarDemapBuf: 生成了一个符号的个数， 比如  5 5 5 然后累加
    //
    ...
    {
        // 输入端 没有数据填充 FACCarDemapBuf FillLevel: 0;
        //cout<<" decode FAC 失败"<<endl;
    }
    //    cout<<"FACMLCDecoder InputBlockSize: "<< FACMLCDecoder.GetInputBlockSize()
    //       <<";FACMLCDecoder OutputBlockSize: "<< FACMLCDecoder.GetOutputBlockSize()
    //      <<";FACCarDemapBuf FillLevel: " <<FACCarDemapBuf .GetFillLevel()
    //      <<"\n;FACDecBuf GetFillLevel: " <<FACDecBuf.GetFillLevel()<<endl;  // 0
...
void CDRMReceiver::UtilizeDRM(bool& bEnoughData)
{

    // 地址竟然不一样 ，都是父类
    // cout<<"2 FACUseBuf %p: "<< static_cast<void*>(&FACUseBuf)<< endl;
    ...
    /*
    平常都是 等于 宏 ，然后有一次会解除 str0  */
    //    cerr <<"--iDataStreamID: "<< iDataStreamID
    //            <<" ; iAudioStreamID:"<< iAudioStreamID << endl;


    /* Data decoding */
    if (iDataStreamID != STREAM_ID_NOT_USED)
    {
        cerr << "Data processing" << iDataStreamID << endl;
    ...
    if( (iDataStreamID == STREAM_ID_NOT_USED) &&
            (iAudioStreamID == STREAM_ID_NOT_USED)) // try and decode stream 0 as audio anyway
    {
        // cout<<"try and decode stream 0 as audio_ "<< endl;
        if (AudioSourceDecoder.ProcessData(Parameters,
                                           MSCUseBuf[0],
                                           AudSoDecBuf))
        {
...
void CDRMReceiver::SetInStartMode()
{
    ...
    /* Define with which parameters the receiver should try to decode the
       ...
       need to reinitialize     定义接收方应该使用哪些参数尝试解码
       信号。如果我们的假设是正确的，接收者就不会 需要重新初始化 */
...
void CDRMReceiver::process()
{
    ...
    else  //非网络
    {
        ...
        if (WriteIQFile.IsRecording())
        {
            ...
            if (SplitForIQRecord.ProcessData(Parameters, RecDataBuf, DemodDataBuf,
                                             IQRecordDataBuf))
            {
                bEnoughData = true;
            }
    ...
    {
    case RM_DRM:
        // DEBUG_COUT("Split the data ");
        SplitFAC.ProcessData(Parameters, FACDecBuf, FACUseBuf, FACSendBuf);

        /* if we have an SDC block, make a copy and keep it until
         * the next frame is to be sent */
...
void CDRMReceiver::process(CSingleBuffer<_BINARY> &AFbuff)
{
    CVectorEx<_BINARY>* pAFbuff = AFbuff.QueryWriteBuffer();
    if(pAFbuff == NULL)
    {
        DEBUG_COUT("子接收机 pAFbuff == NULL " );
    }

    //DEBUG_COUT("(子线程)子接收机 process重载: AFbuff size = "<< pAFbuff->Size());

#if 0
    bool bEnoughData = true;
    /* Input - from upstream RSCI or input and demodulation from sound card / file */

    DecodeRSIMDI.ProcessData(Parameters, AFbuff /*RSIPacketBuf*/, FACDecBuf, SDCDecBuf, MSCDecBuf);
...
CDRMReceiver::InitsForAudParam()
{
    ...
    Parameters.SetNumAudioDecoderBits(
                Parameters.GetStreamLen(iAudioStreamID) * SIZEOF__BYTE);
    AudioSourceDecoder.SetInitFlag();
}
...
void CDRMReceiver::LoadSettings()
{
    ...
    string sDataFilesDirectory = s.Get("Receiver", "datafilesdirectory", Parameters.GetDataDirectory());
    ...
    if(str.empty())
    {
        std::cout<<"rx: UDP  error"<<std::endl;
    }
    else
    {
        DEBUG_COUT("rx: UDP = " << QString::fromStdString(str));
    }
