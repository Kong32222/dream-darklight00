...
//    CVector<_BINARY> biData;
//    biData.Init( 64 * 1024);

    /* Main routine */
    // std::cout<<"-iNumInBits = "<< iNumInBits<< std::endl;
    for (i = 0; i < iNumInBits; i++)
    {
        ...
        // pbiData  size = 399;
        //DEBUG_COUT("pbiData = "<< pbiData<<"\n");
        //std::cout<<"size = "<< pbiData->size() <<std::endl;

        // biData[i] ^= biPRBSbit;     (*pbiData)[398] ^= biPRBSbit;

        //std::cout<<"(*pbiData)["<< i <<"] ^= biPRBSbit;" <<std::endl;
    }
}
...
void CEngergyDispersal::Init(int iNewNumInBits, int iNewLengthVSPP)
{
    ...
    // std::cout<<"iNumInBits = iNewNumInBits "<<iNumInBits <<std::endl;
}
