...
void CMLCEncoder::ProcessDataInternal(CParameter&)
{
    ...
    // DEBUG_COUT("pvecInputData p ="<< pvecInputData<<"\n"); // 第一次使用 pvecInputData
...
void CMLCEncoder::InitInternal(CParameter& TransmParam)
{
    ...
    /**某一次 打印的现象  模式A  频谱占用3
        iiNumInBits = iL[0] + iL[1] + iL[2] = 399
        iCodeRate = 0, 4
        iiNumInBits = iL[0] + iL[1] + iL[2] = 5904
        iCodeRate = 2, 2
        iCodeRate = 7, 7
        iiNumInBits = iL[0] + iL[1] + iL[2] = 72
        iCodeRate = 0, 6

        ***/
    iNumInBits = iL[0] + iL[1] + iL[2];  //iL[2]:  Very strong protected part (VSPP)
    std::cout<< "iiNumInBits = iL[0] + iL[1] + iL[2] = " <<iNumInBits << std::endl;
    ...
    for (i = 0; i < iLevels; i++)
    {
        ...
        /*  iCodeRate = 0, 4
            ...
            */
        std::cout<<"iCodeRate = "<< iCodeRate[i][0] <<", "<< iCodeRate[i][1]<<std::endl;
    }
    ...
    qDebug()<<"iInputBlockSize = "<< iInputBlockSize ;


    // 65    FAC
    // 405 : 表 30   SDC      QAM单元数
    // 2959: 表 42:  MSC模式A 的QAM单元数
    qDebug()<<"iOutputBlockSize QAM单元数= "<< iOutputBlockSize ;
}
