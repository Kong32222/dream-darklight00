                                              ...
                                              CParameter& Parameter)
{
    ...
    if (permissive || CRCObject.CheckCRC((*pbiData).Separate(16)))
    {
        ...
        /* AFS index
            AFS 索引 ：是一个取值在0到15之间的无符号二进制数，
            指出当FAC中“当前复用帧帧号及AFS索引有效标识”= 00 时，
            当前SDC块内容被再次传输之前这段时间间隔内传输的传输超帧数目。
            对于所有的SDC块，AFS 索引值相等。在复用重新配置时，AFS 索引可能发生改变。
...
*/
               ...
               (bError == false) && (iBitsConsumed < iTotNumBitsWithoutCRC))
        {
            ...
            {
                bVersionFlag = true;
                Parameter.reconfigFlag = true;  //##### my
            }
            ...
            -----数据实体类型       4比特
            “数据体”部分长度至少为4比特，“数据体长度”信息在“实体头”中予以标识（不包括最初的4比特）。
            当数据体还有剩余空间时，一个数据结束标记（0x00）将被发送，其余剩余空间做填零处理。
...
{
    /**
    业务标记数据实体应该在每个SDC块中传输以实现接收端对业务标识的快速显示。
    该数据实体版本标记为唯一机制。实体信息如下：
    -短标识     2比特
    -Rfu	      2比特
    -标记       n比特
    ...
    */
    ...
    // qDebug()<<"type:  iTempShortID:"<<iTempShortID ;
                                  ...
                                  const bool)
{
    ...
    int Packetmode = (*pbiData).Separate(1);
...
//    qDebug()<<"Data entity Type 5------\niTempShortID: "<<iTempShortID;
//    qDebug()<<"DataParam.iStreamID: "<<DataParam.iStreamID;
//    qDebug()<<"Packetmode: "<<Packetmode;
//    qDebug()<<"Packet Id: "<<DataParam.iPacketID;
//    qDebug()<<"iPacketLen : "<<DataParam.iPacketLen; //20
    ...
    /*
        ...
        音频模式: 这个值取决于 音频编码 的值
            音频编码= 00:
                  ...
                  10: 立体声
                  11: 保留
            音频编码= 01（CELP）且SBR标志=0:
                 -预留             1比特
                 -CELP_CRC         1比特
        音频编码= 01（CELP）且SBR标志=1：
        -SBR头标志       1 比特.
        -CELP_CRC        1 比特.
        音频编码= 10（HVXC）：
                 -HVXC速率        1比特
        ...
        0：未用CRC
        1：使用CRC
            SBR头标志：指出音频超帧中是否存在SBR头。
        0：不存在SBR头，使用预先定义的SBR构造，见附录I。
        1：存在SBR头，从SBR比特流中的头得到SBR的构造。
        HVXC速率：指出HVXC的速率。
        0: 2 kbit/s.
        1: 4 kbit/s.
        HVXC_CRC：指出是否用了CRC。
        ...
        文本标志: 指出是否携带文本信息
                   0: 不携带文本信息
                   1：携带文本信息（见6.5节）
        增强标志: 指出在另一个信道中是否存在音频增强数据 。
                   0：不存在音频增强数据
                   1：存在音频增强数据
        编码区域: 该区域取决于 “音频编码”和“SBR标志”的值。
        音频编码= 00（AAC）:
        -预留       5比特
        音频编码= 01（CELP）:
        -CELP索引  5比特
        音频编码 = 10 (HVXC) 且 SBR标志 = 0:
        -预留   5 比特.
        音频编码 = 10 (HVXC) 且 SBR标志 = 1:
        -SBR头标志      1 比特.
        如果SBR头标志= 0:
        -sbr_hvxc_预先调整    4 比特.
        如果SBR头标志= 1:
        -预留                 4 比特.
        CELP索引: 该区域指出CELP 的比特率索引，定义见表10、11(见5.4.1.1节)。
        SBR头标志: 指出音频超帧中是否存在SBR头。
        0：不存在SBR头，使用预先定义的SBR构造，见附录I。
        1：存在SBR头，从SBR比特流中的头得到SBR的构造。
        sbr_hvxc_预先调整: 该区域按照I.2节，指出在给定比特率下使用何种SBR预先调整。
        Rfa：该1-比特, 4-比特 和 5-比特 区域 留作将来定义，定义前置0。
