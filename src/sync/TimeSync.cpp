...
/***
该函数的核心目的是初始化时间同步的各种参数和缓冲区，以便在后续处理中使用。
通过调整与采样率相关的参数，设置滤波器，计算符号块大小和历史缓冲区，确保系统能够有效地处理信号的时间同步需求。
**/
void CTimeSync::InitInternal(CParameter& Parameters)
{
    ...
    /* Adjusting GRDCRR_DEC_FACT to sample rate设置希尔伯特滤波器的参数: */
...
# endif
	...
	/* Init Hilbert filter. Since the frequency offset correction was
       ...
       always "VIRTUAL_INTERMED_FREQ" 初始化希尔伯特滤波器:*/
    ...
    iInputBlockSize = iSymbolBlockSize;     /* For the first loop 对于第一个循环 */
