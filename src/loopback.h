...
#ifndef NUM_TAPS_IQ_INPUT_FILT
#define NUM_TAPS_IQ_INPUT_FILT 27
#endif
...
public:
    ...
    RingBuffer* pRingbuff = nullptr;                //  # 环形缓冲区
...
protected:
    ...
    int                         iFreeSymbolCounter;/* OPH: 用于计数帧内符号数量以生成RSCI输出 */

    _REAL HilbertFilt(const _REAL rRe, const _REAL rIm); // Hilbert变换滤波器，计算信号的Hilbert变换

    ////////////////////////////////////////////
    virtual void InitInternal(CParameter& Parameters); // 内部初始化函数，用于配置参数
    virtual void ProcessDataInternal(CParameter& Parameters);  // 内部数据处理函数，处理输入参数
