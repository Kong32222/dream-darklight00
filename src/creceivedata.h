...
/* The RSI output needs 400ms with a 50% overlap, so this needs more space
   I think the RSCI spec is slightly wrong -
using 150 windows consumes just over 400ms, 149 would be exact
RSI输出需要400ms和50%的重叠，所以这需要更多的空间
我认为RSCI规范有点错误——使用150个窗口消耗的时间刚刚超过400ms，确切的说是149个   */
...
protected:
    ...
    CVector<_SAMPLE>		vecsSoundBuffer;   // 读取到 这个buff ( 来源)
