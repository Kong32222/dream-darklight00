...
void CMultColorLED::SetLight(ELightColor color)
{
	...
	case RL_RED:
		/* Red light */
		bFlagRedLi = true;
        TimerRedLight.start(iUpdateTime); //定时: 时间检测 iUpdateTime =时间间隔
		break;
	case RL_GREY:
		// TODO
		break;
...
{
    QPalette newPalette(palette());         // 通过 palette() 方法获取当前调色板的一个副本，然后修改这个副本。这是正确的做法，因为直接修改返回的调色板可能会影响其他使用相同调色板的窗口部件
    newPalette.setColor(QPalette::Window, color);  //修改副本的颜色
    setPalette(newPalette);                 //设置颜色
}
