...
void CDRMPlot::SetScaleColor(const QColor& color)
{
    plot->axisWidget(QwtPlot::xBottom)->setPalette(QPalette(color));
    plot->axisWidget(QwtPlot::yLeft)->setPalette(QPalette(color));

}

/* timer !!! 根据当前的图表类型 CurCharType ，
 * 通过调用不同的绘图函数来更新图表的数据和样式 */
...
//	plot->setTitle(tr("Input Spectrum"));
    ...
    title.setColor(Qt::lightGray);
    plot->setTitle(title);
	plot->enableAxis(QwtPlot::yRight, false);
//	plot->setAxisTitle(QwtPlot::xBottom, tr("Frequency [kHz]"));
    QwtText title1 (tr("频率 [kHz]"));
    title1.setColor(Qt::lightGray);
    plot->setAxisTitle(QwtPlot::xBottom, title1);
	plot->enableAxis(QwtPlot::yLeft, true);
//	plot->setAxisTitle(QwtPlot::yLeft, tr("Input Spectrum [dB]"));
    QwtText title3 (tr("功率 [dB]"));
    title3.setColor(Qt::lightGray);
    plot->setAxisTitle(QwtPlot::yLeft,title3);
...
//	plot->setTitle(tr("FAC Constellation"));
    QwtText title(tr("FAC 星座图"));
    title.setColor(Qt::lightGray); // 设置标题颜色为白色
    plot->setTitle(title);
	plot->enableAxis(QwtPlot::yRight, false);
//	plot->setAxisTitle(QwtPlot::xBottom, tr("Real"));
    QwtText title2(tr("实部"));
    title2.setColor(Qt::lightGray);
    plot->setAxisTitle(QwtPlot::xBottom,title2);
	plot->enableAxis(QwtPlot::yLeft, true);
    QwtText title3(tr("虚部"));
    title3.setColor(Qt::lightGray);
    plot->setAxisTitle(QwtPlot::yLeft, title3);
    ...
    // 设置自定义的轴线绘制器
    CustomScaleDraw *customScaleDraw = new CustomScaleDraw();
    plot->setAxisScaleDraw(QwtPlot::xBottom, customScaleDraw);
...
void CDRMPlot::SetupMSCConst(const ECodScheme eNewCoSc)
{
    ...
    QwtText title3(tr("虚部"));
    title3.setColor(Qt::lightGray);
    plot->setAxisTitle(QwtPlot::yLeft, tr("虚部"));
