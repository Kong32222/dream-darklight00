...
{
    setupUi(this); //public:: UI  so  buyong ui->  neibu meiyou zhizhen weihu
    ...
    connect(chartSelector,
            ...
            this,
            SLOT(OnListSelChanged( QTreeWidgetItem *, QTreeWidgetItem *)));
...
int systemevalDlg::GetRobModeString()
{
    QString strFACInfo;
...
void systemevalDlg::OnTimer()
{
    ...
    if (rx.GetAcquisitionState() == AS_WITH_SIGNAL)  // rx.GetAcquisitionState() == AS_WITH_SIGNAL
    {
    ...
    ValueFreqOffset->setText(QString().setNum(
                                 rx.ConvertFrequency(Parameters.GetDCFrequency()), 'f', 2)
                             + " Hz");
#endif
...
void systemevalDlg::UpdateGPS(CParameter& Parameters)
{
    ...
    if (gps.set&LATLON_SET)
    {
        ...
        qStrPosition = QString(trUtf8("Lat: %1°  Long: %2°")).arg(gps.fix.latitude,
                                          0, 'f', 4).arg(gps.fix.longitude,0, 'f',4);
