...
LiveScheduleDlg::ExtractTime(const CAltFreqSched& schedule)
{
    ...
    if (iStopHours > 24)
    {
        ...
        else
        {
            ...
            /**你遇到的警告是因为 sprintf 函数在 Qt 中已被标记为过时（deprecated）。
             * 这种函数在现代 C++ 编程中被认为不够安全，
             * 因为它容易导致缓冲区溢出。
             * Qt 推荐使用更安全的替代方法来处理字符串格式化操作，如 QTextStream 或 QString::arg()。**/
            if (iDays > 1)
            {
    ...
    {
        //sResult.sprintf() 的使用方式会触发警告，因为 sprintf 已被标记为过时。
        // 你可以将其替换为更安全的 QString::arg() 或其他推荐的方式

//        sResult.sprintf("%02d:%02d-%02d:%02d",
//                        iStartHours, iStartMinutes, iStopHours, iStopMinutes);
        sResult = QString("%1:%2-%3:%4")
                      .arg(iStartHours, 2, 10, QChar('0'))
                      .arg(iStartMinutes, 2, 10, QChar('0'))
                      .arg(iStopHours, 2, 10, QChar('0'))
                      .arg(iStopMinutes, 2, 10, QChar('0'));
...
LiveScheduleDlg::OnTimerUTCLabel()
{
    ...
    // 因为 sprintf 在 Qt 中已经被弃用。为了避免这个警告，建 使用 QString::arg() 来代替 sprintf()。
//    QString strUTCTime = QString().sprintf("%02d:%02d UTC",
//                                           gmtCur->tm_hour, gmtCur->tm_min);
    QString strUTCTime = QString("%1:%2 UTC")
                         .arg(gmtCur->tm_hour, 2, 10, QChar('0'))
                         .arg(gmtCur->tm_min, 2, 10, QChar('0'));
