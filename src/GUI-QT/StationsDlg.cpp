...
void StationsDlg::LoadScheduleView()
{
    ...
    /*QString::sprintf 方法在 Qt 5 及之前的版本中是可用的，但在 Qt 6 中被移除了。为了格式化字符串，
     * 可以使用 QString::arg() 或 QString::number() 方法来替代。*/
//        QString strTimes = QString().sprintf("%04d-%04d",
//                                             station.StartTime(), station.StopTime());
        // 修改后的数据
        QString strTimes = QString("%1-%2")
            .arg(station.StartTime(), 4, 10, QChar('0')) // 用 0 填充到 4 位
            .arg(station.StopTime(), 4, 10, QChar('0'));
