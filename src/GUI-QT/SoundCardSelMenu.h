...
class CFileMenu;
//  QMenu 用于创建和管理菜单。 包含菜单项、子菜单和分隔符
class CSoundCardSelMenu : public QMenu
{
    Q_OBJECT

public:
    CSoundCardSelMenu(CTRx& ntrx, CFileMenu* pFileMenu, QWidget* parent = 0);

protected:
    CTRx&               trx;
    QMenu*				menuSigInput;
    QMenu*				menuInputDev;           //发射机的 音频输入选项
    QMenu*				menuInputSampleRate;
    QMenu*				menuOutputSampleRate;
    QMenu*				menuOutputDev;          //发射机的 音频输出 选项
    QMenu*              menuInputChannel;
    QMenu*              menuOutputChannel;

    // my
    QList<QAction*> outputDeviceActions;  // 存储输出设备的 QAction
    QList<QAction*> inputDeviceActions;   // 存储输入设备的 QAction
...
public slots:
    // slots connected to signals from user  (user)
    ...
    // slots connected to signals from receiver  (receiver)
