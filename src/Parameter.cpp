...
void CParameter::GetActiveServices(set<int>& actServ)
{
    /* Init return vector */
    actServ.clear();
    //DEBUG_COUT("GetActiveServices");  //是可以进来的  并且服务个数是1
    ...
    for (int i = 0; i < MAX_NUM_SERVICES; i++)
    {
        if (Service[i].IsActive())
        {
            ...
            actServ.insert(i);
            //开启两个就是两个服务  都是自己定义的(发射机)
            // DEBUG_COUT("重要 iServiceID= " << Service[i].iServiceID); // printf
        }
...
void CParameter::SetNumOfServices(const size_t iNNumAuSe, const size_t iNNumDaSe)
{
    ...
    GetActiveServices(actServ);  //获取 有多少个服务
