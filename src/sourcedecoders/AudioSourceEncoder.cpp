                                                  ...
                                                  int &iOutputBlockSize)
{
    ...
    else
    {
        ...
        case CAudioParam::AC_AAC:
        {
            ...
            default:
                iTimeEachAudBloMS = 0; // or some default value
                iNumAudioFrames = 0;
        ...
        //是在音频编码过程中计算出来的一个比率，用来表示 编码采样率和输入音频采样率之间的比例关系，
        // 结合了输入音频数据的帧大小和解码器处理块的长度
        /*
            lEncSamprate：编码器的采样率（可能是 48000Hz 或其他采样率），
            Parameters.GetAudSampleRate()：输入音频的采样率
            lNumSampEncIn：编码器输入的样本数量。这个值通常会在音频块的初始化或输入时计算出来，
            表示编码器每次处理的音频样本数。

            AUD_DEC_TRANSFROM_LENGTH：音频解码器转换时使用的参考长度，

         */
        // Parameters.GetAudSampleRate(): 48000
//        if(lEncSamprate == 0 )
//        {
//            lEncSamprate = 48000;
//        }
        // cout<<"AAC lEncSamprate= "<< lEncSamprate <<endl;  //
        _REAL rRatio =
                _REAL(lEncSamprate) / _REAL(Parameters.GetAudSampleRate())
                * _REAL(lNumSampEncIn) / _REAL(AUD_DEC_TRANSFROM_LENGTH);
        /// cout<<"lEncSamprate == "<<lEncSamprate<<endl; //lEncSamprate == 0
        /// cout<<"lNumSampEncIn== "<<lNumSampEncIn <<endl;  //lNumSampEncIn== 1920
        // cout<<"r Ratio == "<<rRatio <<endl;  //r Ratio == 0
        for (int i = 0; i < iNumChannels; i++)
        {
