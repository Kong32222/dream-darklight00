      ...
      bUseReverbEffect(true), codec(nullptr)
{
    ...
    //    std::cout << "编码器[ DecodeAAC = " << bCanDecodeAAC
    //              << " DecodexHE_AAC = " << bCanDecodexHE_AAC
    //              << " DecodeOPUS = " << bCanDecodeOPUS << std::endl;
...
void CAudioSourceDecoder::ProcessDataInternal(CParameter & Parameters)
{
    bool bCurBlockOK;
    ...
    if (bTextMessageUsed)
    {
        ...
        for (int i = 0; i < SIZEOF__BYTE * NUM_BYTES_TEXT_MESS_IN_AUD_STR; i++)
            vecbiTextMessBuf[i] =
                    (*pvecInputData)[iInputBlockSize -
                    SIZEOF__BYTE * NUM_BYTES_TEXT_MESS_IN_AUD_STR + i];
            ...
            if(inputSampleRate != outputSampleRate)
            {
                ...
                if(eDecError==CAudioCodec::DECODER_ERROR_OK) {
                    ...
                    if(iResOutBlockSize != vecTempResBufOutCurLeft.Size()) { // NOOP for AAC, needed for xHE-AAC
                        ...
                        //        std::cout<< "_REAL(outputSampleRate):"<< _REAL(outputSampleRate) <<std::endl;
                        //        std::cout<< "_REAL(inputSampleRate):"<< _REAL(inputSampleRate) <<std::endl;
                        //                        std::cout<< "rRatio:"<< rRatio <<std::endl;  // rRatio:1.5
