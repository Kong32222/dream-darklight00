  ...
  ERobMode eNewRobustnessMode, ESpecOcc eNewSpectOccup, int iSampleRate)
{
    ...
    /**
     * 有用部分（iFFTSizeN）：这是通过调整FFT（快速傅里叶变换）大小后的值，
     * 通常代表一个OFDM（正交频分复用）符号的有用部分。
     *
      保护间隔（iGuardSize）：这是为了防止符号间干扰（ISI，Inter-Symbol Interference）
        而引入的时间间隔。

     **/
    /* Symbol block size is the guard-interval plus the useful part
     传输信号由连续的OFDM符号组成，每个OFDM符号由保护间隔和有用部分信号组成。
     !! 目前可以找到的 size源头， 等于1280
    */
