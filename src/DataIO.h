...
/* Time span used for averaging the audio spectrum. Shall be higher than the
   ...
   用于平均音频频谱的时间跨度。将会高于400 ms DRM音频块   */
...
//my
// 重写
class CMSCReadData : public CTransmitterModul<_SAMPLE, _SAMPLE>
{
public:
    CMSCReadData() {}
    virtual ~CMSCReadData() {}

    virtual void ReadData(CParameter &Parameter, CSingleBuffer<_BINARY> &mscOutputBuffer);


    virtual void ReadData(CParameter &Parameter, CSingleBuffer<_BINARY> &mscInputBuffer,
                                CSingleBuffer<_BINARY> &mscOutputBuffer);

    CVectorEx<_BYTE>*  pvecInput;                 // MSCbuff
    CVectorEx<_BYTE>*  pVecOutput;                 // MSCbuff

    CSingleBuffer<_BINARY> m_MSCBuf;              // 调制使用MSC
