...
#if 0
    std::cout << "vecbidata.size(bits) = " << vecbidata.Size() << std::endl;
    for (int i = 0; i < iLen / SIZEOF__BYTE; i++) // 8个比特为一个字节
    {
        _BINARY byte = 0; // 初始化一个字节

        for (int j = 0; j < 8; j++) // 每次合并8个比特为一个字节
        {
            byte |= (vecbidata[i * 8 + j] << (7 - j)); // 将当前8个比特按位合并到一个字节中
        }
        // 打印一个字节的十六进制表示
...
void CTagItemDecoderSDCChanInf::DecodeTag(CVector<_BINARY>& vecbiTag, const int iLen)
{
    ...
    const int iNumStreams = (iLen - 8) / 3 / SIZEOF__BYTE;  //  流的总个数 根据规范 计算出流的个数

    pParameter->iNumServices = iNumStreams;  //####### 赋值给 参数类中
    ...
    // MSCProtLev.iPartA = 0 MSCProtLev.iPartB = 0
    // cout<<" MSCProtLev.iPartA = "<< MSCProtLev.iPartA << " MSCProtLev.iPartB = "<<MSCProtLev.iPartB <<endl;
    ...
    for (int i = 0; i < iNumStreams; i++)
    {
        ...
        else
        {
            ...
            // 源码函数  (my调用)
            pParameter->SetMSCProtLev(MSCProtLev, bHierarchical);
