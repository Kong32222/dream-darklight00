...
void CDecodeRSIMDI::ProcessDataInternal(CParameter& Parameters)
{
    ...
    {
        // 0    cout<<"A:"<<TagPacketDecoderMDI.TagItemDecoderRobMod.eRobMode << endl;
        Parameters.SetWaveMode(TagPacketDecoderMDI.TagItemDecoderRobMod.eRobMode);
        /// cout<<"Parameters %p = "<<(void*)&Parameters<< endl;
    }
    ...
    // ##################### the MSC data ###########################
    //int iTempStreamLen = 0; //my 自定义 .给下面 MSC 累加个数使用
    ...
    for(size_t i=0; i<vecpvecOutputData.size(); i++)
    {
        ...
        // my  需要这个函数
        //iTempStreamLen += iStreamLen;
        // Parameters.SetNumDecodedBitsMSC(iTempStreamLen);
