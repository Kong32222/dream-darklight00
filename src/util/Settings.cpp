...
void CSettings::Load(int argc, char **argv)
{
    ...
    首先从初始化文件加载设置，然后解析命令行参数。
    命令行参数将覆盖init-file中的设置!*/
...
bool CIniFile::LoadIni(const char *filename)
{
	string section;
	ifstream file(filename);
    if(file.is_open())
    {
        std::cout << "File[Dream.ini]opened successfully." << std::endl;
    }
    else
    {
         std::cout << "File["<<filename<<"]opened fail." << std::endl;
    }
