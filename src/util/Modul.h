...
public:
    ...
    int getInputBlockSize2(){return iInputBlockSize2; }
    int getInputBlockSize3(){return iInputBlockSize3; }
protected:
                                   ...
                                   CBuffer<TOutput>& OutputBuffer)
{
    ...
    /* Call init of derived modul  调用派生模块的init */
    InitThreadSave(Parameter);
            ...
            CBuffer<TOutput>& OutputBuffer)
{
    ...
    //    else
    //    {
    //        return false;  // NO GetRequestFlag
    //    }
    return true;
}
                                      ...
                                      CBuffer<TOutput>& OutputBuffer2)
{
    ...
    /* Init base-class  调用 InitThreadSave(Parameter);   */
    CModul<TInput, TOutput>::Init(Parameter, OutputBuffer); // iOutputBlockSize2 change
...
/* Take an input buffer and split it 2 ways 将一个输入缓冲区分成两部分 */
...
protected:
    ...
    virtual void InitInternal(CParameter& Parameters)
    {
        ...
        this->iOutputBlockSize2 = this->iInputBlockSize;  //基类init(线程安全init)会调用: iOutputBlockSize2给初始化了
    }
