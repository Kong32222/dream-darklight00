...
public:
    ...
    */
    ...
    void FetchNewSampleRate()
    {
        ...
        {
            iSigSampleRate = iNewSoundcardSigSampleRate
                    * iSigUpscaleRatio / iSigDownscaleRatio;
...
#if 1
    size_t iNumServices = 0;         //音频服务 + 数据服务
    //my
    ECodScheme GetMSCCodingScheme()
    {
        return  eMSCCodingScheme;
    }
    ECodScheme  GetSDCCodingScheme()
    {
        return  eSDCCodingScheme;
    }
    int GetFacShortID()
    {
        return iFacShortID;
    ...
    int iFacShortID;        //fac短标识 自定义  (这是从接收机 赋值， 发射机生成的(但是 没有保存)；)
    uint32_t iServiceID;    //24比特 自定义 (可能不兼容)   ----  表示 32 位无符号整数
    bool multiplexerOK = false;
...
public:
    ...
    /* Identifies the current frame. This parameter is set by FAC 标识当前帧。该参数由FAC设置*/
    int iFrameIDTransm;
    int iFrameIDReceiv;
...
protected:
    ...
    int iAudSampleRate;
    int iSigSampleRate;   //default 48000	// Hz
