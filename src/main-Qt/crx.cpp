...
CRx::CRx(CDRMReceiver& nRx, CTRx *trx)
    : CTRx(trx), rx(nRx), eRunState(STOPPED)
{
...
void CRx::run()
{
    qDebug("rx Working thread started");
    try
    {
    ...
    qDebug("Receiver working thread complete  ");   // 接收器 工作线程完成
}
